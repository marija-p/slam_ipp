...
if (debug)
    ...
    disp(['Number of landmarks = ', num2str(nnz([Lmk(:).used]))]);
    disp(['Number of frames = ', num2str(nnz([Frm(:).used]))]);
    disp(['Robot index = '])
    disp(Rob(1).state.r')
    disp(['Landmarks index = '])
    for i = 1:nnz([Lmk(:).used])
        disp(Lmk(i).state.r')
    end
    disp(['Frames index = '])
    for i = 1:nnz([Frm(:).used])
        disp(Frm(i).state.r')
    end
    
    figure;
    set(gcf, 'Position', [908   497   881   305])
    subplot(1,2,1)
    imagesc(Map.R)
    colorbar
    pbaspect([1 1 1])
    ...
    pbaspect([1 1 1])
    title('P')
    [~, rob_ind] = ismember(Rob(1).state.r, Map.pr);
    x1=rob_ind(1);
    x2=rob_ind(end);
    y1=rob_ind(1);
    y2=rob_ind(end);
    ...
    axis([1, nnz(Map.used), 1, nnz(Map.used)])
    keyboard
    close(gcf)
