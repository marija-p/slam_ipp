function [varargout] = gpUI(hyp, inf, mean, cov, lik, x, y, N_gauss, s2, xs, ys)
% Gaussian Process inference and prediction with uncertain input test data.
% ---
% The gp function provides a flexible framework for Bayesian inference 
% and prediction with Gaussian processes for scalar targets,
% i.e. both regression and binary classification.
% The prior is Gaussian process, defined through specification
...
%   N_gauss  number of sample points in Gauss-Hermite quadrature
%   s2       D by D covariance matrix for training inputs
...
if size(y,2)>1      % deal with (independent) multivariate output y by recursing
  ...
  for i=1:d
    ...
    if nargin>9, in = {in{:}, xs}; end
    if nargin>10, in = {in{:}, ys(:,i)}; end
...
if nargin==9                                     % if no test cases are provided
  ...
  while nact<ns               % process minibatches of test cases to save memory
    ...
    kss = feval(covfunc{:}, hyp.cov, xs(id,:), 'diag');              % self-variance
    if strcmp(cstr,'FITC') || strcmp(cstr,'apxSparse')    % cross-covariances
      Ks = feval(covfunc{:}, hyp.cov, x, xs(id,:)); Ks = Ks(nz,:); % res indep. of x
    else
      Ks = feval(covfunc{:}, hyp.cov, x(nz,:), xs(id,:));        % avoid computation
    end
