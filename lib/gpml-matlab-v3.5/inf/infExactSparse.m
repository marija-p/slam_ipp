function [post, nlZ, dnlZ] = infExactSparse(hyp, mean, cov, lik, x, y)
...
if sn2<1e-6                        % very tiny sn2 can lead to numerical trouble
  ...
  pL = (K+sn2*eye(n)) \ eye(n);                            % L = -inv(K+inv(sW^2))
  alpha = ((K+sn2*eye(n)) \ (y-m))/sl;
  Q = pL/sl - alpha*alpha';     % precompute for convenience
else
  L = chol(K/sn2+eye(n)); sl = sn2;                       % Cholesky factor of B
  pL = L;                                           % L = chol(eye(n)+sW*sW'.*K)
  alpha = ((K/sn2+eye(n)) \ (y-m))/sl;
  Q = ((K/sn2+eye(n)) \ eye(n))/sl - alpha*alpha';     % precompute for convenience
end
