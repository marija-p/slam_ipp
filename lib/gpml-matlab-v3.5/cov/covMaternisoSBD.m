function K = covMaternisoSBD(d, hyp, x, z, i)
...
if dg                                                               % vector kxx
  ...
  if xeqz                                                 % symmetric matrix Kxx
    Dist = sqrt(sq_dist(x')); Dist(Dist <= 6)=1; Dist(Dist > 6)=0;
%     K = sparse(sqrt(d)/ell * K);
    K = sqrt(sq_dist(sqrt(d)/ell*x'));
  else                                                   % cross covariances Kxz
    K = sqrt(sq_dist(sqrt(d)/ell*x',sqrt(d)/ell*z'));
  end
end
